...
LF.NotificationService/          # Background email delivery (no gRPC, no public API) — Quartz.NET EmailDispatchJob polls the LFEmailMessages outbox and sends via MailKit SMTP; see "Email" in Tech Stack
...
- **Unleash** — feature flags, replacing what used to be a DB-backed `PlatformSettings` kill-switch plus an admin settings screen (both removed; see `Architecture.md`). The `Unleash.Client` package lives in `LF.Infrastructure` behind `LF.Application`'s `IFeatureFlagService`; flag names are constants in `LF.Application/Common/FeatureFlags.cs` (today: `lf.self_enrollment`). Registered via `AddInfrastructureFeatureFlags(configuration)` in **`LF.WebApi` only** — the three gRPC hosts sit on an egress-less internal Docker network and cannot reach the Unleash server, which is why the self-enrollment guard lives in `EnrollmentLearningService` (both `POST /api/enrollments` and `POST /api/payments/checkout` funnel through it) rather than in `LF.CourseService`. **Fails closed**: when `Unleash:ApiUrl`/`Unleash:ApiKey` are missing or still `"CHANGE_ME"`, a `DisabledFeatureFlagService` is registered and every flag reads `false`. The API key is never committed — `dotnet user-secrets` in dev, `Unleash__ApiKey` via `.env` in Docker, `UNLEASH_API_KEY` as an Aspire secret parameter. When adding a flag, add the constant to `FeatureFlags` rather than passing a string literal.

- **Email (outbox + `LF.NotificationService`)** — to send an email, inject `IEmailQueue` (`LF.Application/Services/Email/`, registered in `AddAuthenticationApplication()`, i.e. LF.WebApi) and call `EnqueueAsync`: it only inserts an `EmailMessage` row (`LFEmailMessages`, optional `NotBefore` for delayed sends). Delivery happens in the separate `LF.NotificationService` host: a Quartz.NET (v4) `[DisallowConcurrentExecution]` `EmailDispatchJob` on a cron (`EmailDispatch:Cron`, default every 30s) runs `IEmailDispatchService.DispatchDueAsync`, which sends through `IEmailSender` (MailKit `MailKitEmailSender` in `LF.Infrastructure`, via `AddInfrastructureEmail(configuration, out isConfigured)`). Retry/backoff/max-attempts logic lives on the `EmailMessage` entity (`RecordFailure`); the sender maps transport errors to `EmailDeliveryException(IsTransient)` — SMTP 5xx is permanent, everything else retries. **Fails closed**: when `Smtp:Host`/`Smtp:FromAddress` are blank or `CHANGE_ME`, the job is never scheduled and queued mail stays Pending. Single replica by design (no row locking). SMTP secrets: AppHost user-secrets `SMTP_HOST`/`SMTP_USERNAME`/`SMTP_PASSWORD`/`SMTP_FROM_ADDRESS` in dev, `Smtp__*` in `.env` for Docker (the container joins `leanforge-public` for SMTP egress). Admins can check delivery end to end with `POST /api/admin/email/test` and `GET /api/admin/email/{id}`.
  - **Templates + language**: email content lives in `LF.Application/Templates/Email/{Name}.{ru|en}.html` (+ optional `.txt`), embedded with `WithCulture="false"` (without it MSBuild moves `*.ru.html` into a satellite assembly). `IEmailTemplateRenderer` takes the subject from `<title>`, HTML-encodes `{{Placeholder}}` values in the body only, throws on a missing value, and falls back to `UserLanguage.Default` (`ru`). Each user's language is `DbUser.PreferredLanguage` (`LF.AppDomain/Models/User/UserLanguage.cs`, null until chosen). It's set through `PUT /api/profile/language` → `UpdateUserLanguage` gRPC → IdentityService. The SPA's `authStore.changeLocale` (locale toggle) saves it, and `ensureInitialized` applies the stored value or uploads the current locale when it's unset.
  - **Enrollment confirmation**: `IEnrollmentNotifier.StageEnrollmentConfirmationAsync` adds the email to the DbContext *without saving*, so it commits in the same `SaveChangesAsync` as the enrollment (atomic outbox). It's called wherever an enrollment becomes Active: free `EnrollmentService.EnrollAsync`, paid `ActivatePaidEnrollmentAsync` (not for PendingPayment, and not on idempotent replays), and admin `CourseService.EnrollUserAsync`, all running in LF.CourseService. Links use `AppUrlOptions` (`App:PublicBaseUrl`, validated on start in LF.CourseService). New transactional emails should follow the same pattern: a template pair plus a notifier that stages into the caller's unit of work.
  - **Course-update digests**: `CourseService` calls `ICourseChangeTracker` when a lesson is added, or when its title, content or parts *actually* change. `Lesson.Rename`/`UpdateContent`/`ReplaceParts` return `bool`, and `ReplaceParts` compares a fingerprint of the student-visible content because the editor rebuilds every part on each save. Reordering and the preview flag are not tracked. The tracker stages a `CourseContentChange` (`LFCourseContentChanges`, cascade-deleted with its lesson) for published courses only, merging into the lesson's single pending row. `CourseUpdateDigestJob` in LF.NotificationService (`CourseUpdateDigest:Cron`/`QuietPeriodMinutes`/`MaxCoursesPerRun`) sends one digest per course once its newest pending change is older than the quiet period (default 30 min), so an editing session becomes one email. Recipients are Active enrollments that existed before the last change, each emailed in their own language with a `/courses/learn/{enrollmentId}` link. The emails and the `NotifiedAt` marks commit in one `SaveChanges` per course. Changes to a course that is unpublished at digest time are discarded. The digest job runs even without SMTP (it only writes outbox rows). Templates can use `{{{Key}}}` for raw, caller-encoded markup (the lesson list); everything else stays `{{Key}}`.
